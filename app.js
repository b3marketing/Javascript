...
for (let i = 0; i < dino["Dinos"].length; i++) {
    let currentDino = dino["Dinos"][i]
        //Create a new dino from the Dino Constructor
    const myDino = new Dino(currentDino.species, currentDino.weight, parseInt(currentDino.height), currentDino.diet, currentDino.where, currentDino.when, currentDino.fact);
...
function toInches() { // takes feet and inches from an object and creates a new property on the object called height
...
function buildDinoTiles() {
    ...
    for (let i = 0; i < dinoArr.length; i++) { // Iterate over the dinoArr and add each card to an array called cardArr
        let currentDino = dinoArr[i]; // Get the current dino
        let title = currentDino.species; // Set variables to be used in cards equal to currentDino properties
        let img = "images/" + currentDino.species.toLowerCase() + ".png"; // look up PNG using the species name
        let fact = facts[Math.floor(Math.random() * facts.length)]; // grab a random item from facts array. 
        ...
        let card = '<div class="grid-item"> <h3>' + title + '</h3> <img src="' + img + '"></img> <p>Fact: ' + fact + '<br></p> </div>'; // Build the card
        cardArr.push(card);
    }
    return cardArr;
}
...
function buildHumanTile() { // Creates a tile from the human object
    let img = "images/human.png";
    let title = human.name;

    let humanCard = '<div class="grid-item"> <h3>' + title + '</h3> <img src="' + img + '"></img></div>';
    return humanCard;
}
...
function addTilesToDOM(cardArr, humanTileResult) { // Takes the array cardArr and appends each card to the inner html of the grid
    ...
    for (let i = 0; i < cardArr.length; i++) {
        document.getElementById("grid").innerHTML += cardArr[i];
    }
}
